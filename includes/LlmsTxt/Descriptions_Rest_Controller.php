 ...
 *
 * Five routes under `agentready/v1/llms-txt/descriptions/*`, all gated
 * by `manage_options`:
 *
 *   GET    /agentready/v1/llms-txt/descriptions
 *   PATCH  /agentready/v1/llms-txt/descriptions/<post_id>
 *   DELETE /agentready/v1/llms-txt/descriptions/<post_id>/manual
 *   POST   /agentready/v1/llms-txt/descriptions/<post_id>/regenerate
 *   POST   /agentready/v1/llms-txt/descriptions/bulk-regenerate-stale
 *
...
final class Descriptions_Rest_Controller {
	...
	public static function handle_bulk_regenerate_stale( \WP_REST_Request $request ) {
		...
		if ( empty( Context_Profile_Settings::get_profile()['llm_descriptions_enabled'] ) ) {
			return new \WP_Error(
				...
				\__( 'LLM descriptions are disabled in the Context Profile.', 'agentready' ),
				array( 'status' => 409 )
			);
		}
	...
	public static function check_permission() {
		if ( ! \current_user_can( 'manage_options' ) ) {
			return new \WP_Error(
				'rest_forbidden',
				\__( 'You are not allowed to manage entry descriptions.', 'agentready' ),
