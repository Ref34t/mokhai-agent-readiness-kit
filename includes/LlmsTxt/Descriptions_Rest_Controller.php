...
final class Descriptions_Rest_Controller {
	...
	public static function handle_bulk_regenerate_stale( \WP_REST_Request $request ) {
		...
		if ( empty( Context_Profile_Settings::get_profile()['llm_descriptions_enabled'] ) ) {
			return new \WP_Error(
				...
				\__( 'LLM descriptions are disabled in the Context Profile.', 'agentable' ),
				array( 'status' => 409 )
			);
		}
	...
	public static function check_permission() {
		if ( ! \current_user_can( 'manage_options' ) ) {
			return new \WP_Error(
				'rest_forbidden',
				\__( 'You are not allowed to manage entry descriptions.', 'agentable' ),
	...
	private static function resolve_post( int $post_id ) {
		if ( $post_id <= 0 ) {
			return new \WP_Error(
				'rest_invalid_post',
				\__( 'A valid post ID is required.', 'agentable' ),
				array( 'status' => 400 )
			);
		}
