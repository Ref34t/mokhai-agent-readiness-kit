...
final class Engine {
	...
	private static function score_schema_coverage( array $signals ): array {
		...
		if ( '' !== $plugin ) {
			...
			self::add_reason( $reasons, $reason_keys, 'sc_native_jsonld', 'AI Readiness Kit is emitting native JSON-LD (WebSite + Organization + per-content). Schema coverage satisfied without a third-party SEO plugin.' );
			$value = 100;
		} else {
			...
			self::add_reason( $reasons, $reason_keys, 'sc_no_structured_data', 'No structured data detected on this site. Enable Schema emission in the Context Profile to have AI Readiness Kit emit native JSON-LD, or rely on a third-party SEO plugin.' );
			$value = 60;
		}
