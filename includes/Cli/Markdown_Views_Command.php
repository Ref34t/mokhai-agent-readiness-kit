...
final class Markdown_Views_Command {
	...
	public function preview( array $args, array $assoc_args ): void {
		...
		if ( null === $post ) {
			\WP_CLI::error( \sprintf( /* translators: %s is the user-supplied target. */ \__( 'No post matches "%s".', 'ai-readiness-kit' ), $target ) );
		}
		...
		if ( ! Context_Profile_Settings::is_module_enabled( 'markdown_views' ) ) {
			\WP_CLI::error( \__( 'Markdown Views is disabled in the Context Profile.', 'ai-readiness-kit' ) );
		}
			...
			if ( null !== $reason ) {
				\WP_CLI::error(
					\sprintf(
						...
						\__( 'Post #%1$d is not exposable (reason: %2$s). Use --bypass-exposure to preview anyway (requires manage_options).', 'ai-readiness-kit' ),
						(int) $post->ID,
						$reason
					)
				);
	...
	private static function print_cache_meta( int $post_id ): void {
		...
		if ( ! \is_array( $row ) ) {
			\WP_CLI::warning( \__( 'cache: miss (no row)', 'ai-readiness-kit' ) );
			return;
		}
		...
		\WP_CLI::warning(
			\sprintf(
				...
				\__( 'cache: hit | hash=%1$s | walker_version=%2$s | generated_at=%3$s', 'ai-readiness-kit' ),
