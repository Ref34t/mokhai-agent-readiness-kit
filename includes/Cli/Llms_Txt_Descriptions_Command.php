 ...
 *
 * Three subcommands mounted at `wp agentready llms-txt descriptions`:
 ...
 *     $ wp agentready llms-txt descriptions status
 *
 *     # Queue a description job for every missing entry across all
 *     # exposed CPTs and statuses.
 *     $ wp agentready llms-txt descriptions backfill
 *
 *     # Force-regenerate the cache for one post.
 *     $ wp agentready llms-txt descriptions regen 42
 */
final class Llms_Txt_Descriptions_Command {
	...
	public static function register(): void {
		...
		\WP_CLI::add_command( 'agentready llms-txt descriptions', self::class );
	}
	...
	public function regen( array $args, array $assoc_args ): void {
		...
		if ( array() === $args ) {
			\WP_CLI::error( 'Usage: wp agentready llms-txt descriptions regen <post_id>' );
			return;
		}
