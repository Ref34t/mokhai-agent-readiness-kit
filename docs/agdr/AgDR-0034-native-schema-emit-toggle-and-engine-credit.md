...
elseif ( $native )    { $value = 100; $reasons[] = 'Mokhai is emitting native JSON-LD (WebSite + Organization + per-content). Schema coverage satisfied without a third-party SEO plugin.'; }
...
- Operators upgrading from PR #75 → #73 lose the always-on baseline emission. View-source diff: lose 4 JSON-LD nodes on the front-end. The Context Score's `schema_coverage` reason text changes from "Mokhai native emission active" to "Enable Schema emission in Context Profile" until the operator opts in.
