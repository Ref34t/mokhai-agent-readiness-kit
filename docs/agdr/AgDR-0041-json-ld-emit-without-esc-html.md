...
Net effect: `schemaCoordination.filled` in Context Score reports `["WebSite", "Organization", "WebPage", "Article"]` (or similar) and Score's narrative claims *"Native JSON-LD is enabled for WebSite, Organization, and per-content schema"* — accurate at the PHP node-array layer, completely false at the wire layer. Every site that activated Mokhai between PR #75 (v0.1.0) and this fix shipped broken structured data.
...
- Ref34t/mokhai-agent-readiness-kit#118 — the bug report (filed 2026-05-24 during v0.1.1 post-release verification).
